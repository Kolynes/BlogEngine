    ...
    <md-card>
        <md-toolbar class="md-transparent md-dense md-layout" :md-elevation="0">
            <div class="md-layout-item md-size-100" style="padding-top: 10px">
                <span class="md-subheading" > <md-icon class="md-size-2x">collections</md-icon> Gallery</span>
            </div>
            <div class="md-layout-item md-size-100">
                <md-field  md-clearable >
                    <label>Find an image</label>
                    <md-input v-model="searchString"/>
                <md-button v-if="!searchString" class="md-icon-button md-primary" @click="showSaveToGalleryDialog = true">
                    <md-icon>add</md-icon>
                </md-button>
                </md-field>
            </div>
        </md-toolbar>
        <infinite-scroller id="gallery-list-content" :class="{interfaced}" :refresh.sync="refresh" :showLoading="loading" :loader="loadGallery">
            <template slot-scope="{item}">
                <center>
                    <md-card class="gallery-image" md-with-hover>
                        <md-card-media-cover md-solid>
                            ...
                            <md-card-area>
                                <md-card-header>
                                    <span style="width: 100%; white-space: nowrap; text-overflow:ellipsis; display: block; overflow: hidden">{{item.text}}</span><br>
                                    <span >{{item.date}}</span>
                                </md-card-header>
                                <md-card-actions>
                                    <md-button class="md-icon-button" @click="editGalleryImage(item)">
                                        <md-icon>edit</md-icon>
                                    </md-button>
                                    <md-button class="md-icon-button" @click="galleryImageToOperateOn = item.pk; showDeleteDialog = true">
